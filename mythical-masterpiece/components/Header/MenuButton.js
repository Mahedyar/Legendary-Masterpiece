...
export default function MenuButton(){
    ...
    return(
        <>
            {open.show && <ModalMenu onConfirm={handleClose} onOpen={open}/>}
            <div style={{ width: '100%' }}>
                <Box
                ...
                >
                    {ItemsMenu.map((item)=>
                        <Item onMouseEnter={()=>hoverIcon(item.id)}  onMouseOver={handleOpen} className={'btn-menu'} key={item.id}>
                            {item.title}{show.show && show.id===item.id?
                            <ExpandMoreIcon className={'icon-menu'}  sx={{fontSize:10.5}}/>:
